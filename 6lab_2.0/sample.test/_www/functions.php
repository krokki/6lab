...
function render_all_film_for_cinema($data, $id_cinema)
{
    ...
    for ($i = 0; $i < count($data); $i++) {
        ...
        foreach ($data[$i] as $row)
        {
            if (($row !== $data[$i]['film_id']) AND ($row !== $data[$i]['file']) AND ($row !== $data[$i]['cinema_id'])) {

                echo '<div class="col item-table font-style">
                                 <div class="row align-items-center justify-content-center family">
                                      <div class="col"><a href="' . $url_parent . '">' . $row . '</a></div>
                                 </div>
                      </div>';
            }
        }
        ...
        echo ' <div class="col item-table font-style" style="height: inherit;">
                 <div class="row align-items-center justify-content-center family">
                    <button class="btn-info"><a style="text-decoration: none; color: black" href="' . $url_edit . '">Edit</a></button>
                    <button class="btn-danger btn_del_f_f_c" data-id="' . $data[$i]['film_id'] . '">delete</button>
                 </div>
               </div>
        </div>';
